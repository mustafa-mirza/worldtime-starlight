...
parser.add_argument('--templateName', help='(Optional) Set the template name to integrate Threat Dragon model with provided template. Applicable only with reportType application_model_threat_dragon and application_model_threat_dragon_plus.')
...
def generate_report(access_token, customer_id, department_id, start_timestamp, end_timestamp, report_format, report_type, template_name):
...
def execute():
    ...
    if Arg.domain:
        ...
        if template_name and (str(Arg.report_type).lower() == 'application_model_threat_dragon' or str(
            ...
            print_error("Threat Draon template with temaplate name " + template_name + " not found for domain " + Arg.domain + " at Orchestrator")
            template_name = None
        unique_id = generate_report(get_access_token(), customer_id, department_id, Arg.start_timestamp, Arg.end_timestamp, Arg.format, Arg.report_type, template_name)
    else:
        unique_id = generate_report(get_access_token(), None, None, Arg.start_timestamp, Arg.end_timestamp, Arg.format, Arg.report_type, None)
